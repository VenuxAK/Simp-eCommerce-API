...
class InvoiceTest extends TestCase
{
    ...
    {
        $response = $this->getJson('/api/v1/invoices?status=issued', $this->headers);
        $response->assertOk()->assertJsonCount(1, 'data');
    }
    ...
    public function test_can_show_invoice(): void
    {
        ...
        $response = $this->getJson("/api/v1/invoices/{$invoice->id}", $this->headers);
    ...
    {
        $invoice = Invoice::first();
        $response = $this->getJson("/api/v1/invoices/{$invoice->id}/print", $this->headers);
    ...
    {
        $invoice = Invoice::first();
        $this->patchJson("/api/v1/orders/{$this->orderId}/status", ['status' => 'cancelled'], $this->headers);
