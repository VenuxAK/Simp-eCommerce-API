...
class UserTest extends ApiTestCase
{
    ...
    {
        $this->getJson('/api/v1/users', $this->adminHeaders)->assertOk();
    }

    public function test_staff_cannot_list_users(): void
    {
        $this->getJson('/api/v1/users', $this->staffHeaders)->assertForbidden();
    }
    ...
    public function test_admin_can_update_user(): void
    {
        ...
        $this->putJson("/api/v1/users/{$user->id}", ['name' => 'Updated'], $this->adminHeaders)->assertOk();
    }

    public function test_staff_cannot_update_user(): void
    {
        $user = User::factory()->create();
        $this->putJson("/api/v1/users/{$user->id}", ['name' => 'Hacked'], $this->staffHeaders)->assertForbidden();
    }

    public function test_admin_cannot_delete_self(): void
    {
        $admin = User::role('root')->first();
        $this->deleteJson("/api/v1/users/{$admin->id}", [], $this->adminHeaders)->assertUnprocessable();
    }

    public function test_admin_cannot_delete_another_admin(): void
    {
        $admin2 = User::factory()->root()->create();
        $response = $this->deleteJson("/api/v1/users/{$admin2->id}", [], $this->adminHeaders);
    ...
    public function test_admin_cannot_delete_user_with_orders(): void
    {
        ...
        $this->deleteJson("/api/v1/users/{$staff->id}", [], $this->adminHeaders)->assertUnprocessable();
    ...
    public function test_admin_can_delete_user_without_orders(): void
    {
        ...
        $this->deleteJson("/api/v1/users/{$staff->id}", [], $this->adminHeaders)->assertOk();
    ...
    {
        $user = User::factory()->create();
        $this->deleteJson("/api/v1/users/{$user->id}", [], $this->staffHeaders)->assertForbidden();
    }
