...
> Canonical API documentation for the dashboard SPA and storefront clients.  
> **112 routes** across 15 module route files.
...
| POST | `/products` | Admin | `{ category_id, brand_id?, name, base_price, description?, supplier_id?, variants: [...] }` |
...
| POST | `/categories` | Admin | `{ name, description?, parent_id? }` |
| PUT | `/categories/{id}` | Admin | Update |
| DELETE | `/categories/{id}` | Admin | Delete (fails if has products) |
| POST | `/categories/{id}/image` | Admin | Multipart `image` file upload |

### A.7 Brands

| Method | Endpoint | Role | Body / Notes |
|--------|----------|------|--------------|
| GET | `/brands` | Staff | List all |
| GET | `/brands/{id}` | Staff | Single |
| POST | `/brands` | Admin | `{ name }` — slug auto-generated |
| PUT | `/brands/{id}` | Admin | Update |
| DELETE | `/brands/{id}` | Admin | Delete (fails if has products) |
| POST | `/brands/{id}/logo` | Admin | Multipart `logo` file upload |
...
| GET | `/storefront/brands` | — | `{ data: Brand[] }` |
| GET | `/storefront/settings` | — | `{ data: StoreSettings }` |

**Product** includes: `id`, `name`, `slug`, `description`, `base_price`, `image_url`, `total_stock`, `brand` (nested), `category` (nested), `variants[]` (nested with `sku`, `size`, `color`, `price_adjustment`, `stock_quantity`, `image_url`).

**Category** includes: `id`, `name`, `slug`, `description`, `parent_id`, `products_count`.

**Brand** includes: `id`, `name`, `slug`, `logo`.
...
**CartItem**: `id`, `product_variant_id`, `quantity`, `variant` (nested with `product`).  
